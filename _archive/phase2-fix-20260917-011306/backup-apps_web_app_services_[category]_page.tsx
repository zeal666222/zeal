...
// Uses: service registry + live consultant counts
// ═══════════════════════════════════════════════════════════════════════════════
...
export default async function CategoryPage({ params }: PageProps) {
  ...
  // Query all consultants in this category once
  const admin = createAdminClient();
  const { data: consultants } = await admin
    .from("Consultant")
    .select("id, specialties, rating, perMinuteRate")
  ...
  for (const svc of services) {
    const matched = (consultants ?? []).filter((c) =>
      (c.specialties ?? []).some((s: string) =>
        svc.specialties.includes(s) || s.toLowerCase() === svc.displayName.toLowerCase()
      )
    );
    const count = matched.length;
    const avgRating = count > 0
      ? matched.reduce((sum, c) => sum + (c.rating ?? 0), 0) / count
      : 0;
    const minPrice = count > 0
      ? Math.min(...matched.map((c) => c.perMinuteRate ?? 0))
      : 0;
    ...
    <div className="max-w-6xl mx-auto px-4 py-8">
      ...
      <div className="mb-8">
        ...
        <p className="text-slate-400">
          {consultants?.length ?? 0} verified guide{consultants?.length !== 1 ? "s" : ""} · {services.length} specializations
        </p>
      </div>
