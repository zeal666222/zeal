...
import { Search, MessageCircle, Sparkles, ChevronRight } from "lucide-react";
      ...
      <div className="p-3">
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-9 pr-4 py-2 bg-slate-900/90 border border-white/5 rounded-xl text-xs text-slate-200 placeholder:text-slate-500 focus:outline-none focus:border-[#9D7DC5]/50"
          />
        </div>
      </div>
      ...
      {online.length > 0 && (
        <div className="py-2 px-4 border-b border-white/5 flex items-center gap-3 overflow-x-auto hide-scrollbar">
          {online.map((p) => (
            <Link
              key={p.sessionId}
              href={`/consultant/chat/${p.sessionId}`}
        ...
        {filtered.length === 0 ? (
          <div className="h-64 flex flex-col items-center justify-center p-6 text-center">
            ...
            <p className="text-xs text-slate-500 mt-1 max-w-[220px]">
              Seekers who start a chat or book a session will appear here.
            </p>
          </div>
        ) : (
          filtered.map((conv) => {
            const isActive = pathname === `/consultant/chat/${conv.sessionId}`;
            const isMeLast = conv.lastMessageSenderId === currentUserId;
            return (
              <Link
                key={conv.sessionId}
                href={`/consultant/chat/${conv.sessionId}`}
                className={cn(
                  "flex items-center gap-3.5 px-4 py-3.5 transition-colors group",
                  isActive ? "bg-[#9D7DC5]/15 border-l-4 border-[#9D7DC5]" : "hover:bg-white/[0.03]",
                )}
              >
