// ZEAL — Rate Limiting (PostgreSQL, zero external keys)
...
async function checkLimit(c: LimiterConfig, id: string): Promise<RateLimitResult> {
  const sb = getClient();
  const failed: RateLimitResult = {
    success: process.env.NODE_ENV !== "production",
    limit: c.tokens, remaining: 0,
    reset: Date.now() + c.windowSeconds * 1000, count: 0,
  };
  if (!sb) { console.warn("[rate-limit] no client"); return failed; }
  try {
    ...
    if (error || !data) { console.error("[rate-limit]", error?.message); return failed; }
    return data as RateLimitResult;
  } catch (e) { console.error("[rate-limit]", e); return failed; }
}
...
export const aiTokenBudget   = createLimiter(20000, 3600, "ai-budget");
...
export const enforceRateLimit = async (i?: unknown) =>
  generalLimiter.limit(typeof i === "string" ? i : "anonymous");
