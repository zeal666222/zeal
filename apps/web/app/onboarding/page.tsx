...
import { Sparkles, IndianRupee, ArrowRight, Loader2, Calendar, User, Moon, FastForward } from "lucide-react";
...
export default function OnboardingPage() {
  ...
  const handleSkip = async () => {
    ...
    if (res.success) {
      ...
      setErrorMsg(res.error || "Failed to skip onboarding.");
      setSkipping(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-50 flex items-center justify-center p-4 relative overflow-hidden">
      ...
      <div className="max-w-lg w-full bg-slate-900/80 backdrop-blur-2xl border border-white/10 rounded-[2.5rem] p-8 sm:p-12 shadow-2xl relative z-10">
        ...
        {/* STEP 1: Metaphysical Profile (Now Skippable) */}
        {step === 1 && (
          <div className="animate-in fade-in slide-in-from-right-8 duration-500">
            <div className="text-center mb-8">
              <h1 className="text-3xl font-black tracking-tight">Your Energy Profile</h1>
              <p className="text-slate-400 text-sm mt-2">Align your account for better consultations, or skip this step to explore first.</p>
            </div>
            ...
            <form onSubmit={(e) => { e.preventDefault(); handleNext(); }} className="space-y-5">
              ...
              <div className="flex flex-col sm:flex-row gap-3 pt-2">
                <button 
                  ...
                  onClick={handleSkip}
                  disabled={skipping}
                  className="flex-1 py-4 bg-transparent hover:bg-white/5 border border-transparent hover:border-white/10 text-slate-400 hover:text-white rounded-2xl font-bold text-sm transition-all cursor-pointer flex items-center justify-center gap-2"
                >
                  {skipping ? <Loader2 size={16} className="animate-spin" /> : "Skip for Now"} <FastForward size={16} />
                </button>
                <button 
                  type="submit" 
                  className="flex-1 py-4 bg-white/5 hover:bg-white/10 border border-white/10 text-white rounded-2xl font-bold text-sm transition-all cursor-pointer flex items-center justify-center gap-2"
                >
        ...
        {step === 2 && (
          <form onSubmit={handleSubmit} className="animate-in fade-in slide-in-from-right-8 duration-500 flex flex-col items-center text-center">
            {/* ... Keep the exact same bonus reveal UI from before ... */}
