"use client";
// ═══════════════════════════════════════════════════════════════════════════════
// Consultant Earnings — balance, escrow, transactions, withdrawal
// ═══════════════════════════════════════════════════════════════════════════════
...
import { WithdrawalModal } from "@/components/consultant/WithdrawalModal";
  ...
  const stats = useMemo(() => {
    ...
    const last7 = txs
      .filter(
        (t) =>
          t.amount > 0 &&
          new Date(t.createdAt) > new Date(Date.now() - 7 * 864e5),
      )
      .reduce((s, t) => s + t.amount, 0);
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-slate-400 mt-1">
          Real-time wallet and payout history
        </p>
      </div>
      ...
      <div className="relative overflow-hidden p-6 lg:p-8 bg-gradient-to-br from-emerald-950/60 via-slate-900 to-slate-950 border border-emerald-500/30 rounded-3xl shadow-2xl">
        ...
        <div className="relative z-10">
          ...
          {/* Withdraw button — visible only when balance >= ₹100 */}
          <button
            onClick={() => setShowWithdraw(true)}
            disabled={balance < 100}
            className="mt-4 px-5 py-2.5 bg-white/10 hover:bg-white/20 border border-white/20 rounded-xl text-sm font-bold text-white disabled:opacity-40 disabled:cursor-not-allowed transition-all"
          >
            Withdraw Funds
          </button>
            ...
            <div className="mt-4 pt-4 border-t border-white/5 flex gap-6 text-xs">
              {escrow > 0 && (
                <span className="text-amber-400">
                  In escrow:{" "}
                  <strong className="font-mono">₹{escrow.toFixed(2)}</strong>
                </span>
              )}
              {pendingOut > 0 && (
                <span className="text-slate-400">
                  Pending withdrawal:{" "}
                  <strong className="font-mono">₹{pendingOut.toFixed(2)}</strong>
                </span>
              )}
            </div>
          )}
