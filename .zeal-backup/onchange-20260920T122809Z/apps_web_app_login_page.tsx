...
import {AlertCircle, ArrowRight, Check, Compass, Eye, EyeOff, Loader2, Lock, Mail, Sparkles, Briefcase} from "lucide-react";
...
const ERROR_COPY: Record<string, string> = {
  ...
  NOT_AUTHORIZED: "This account does not have access.",
};
...
function Content() {
  ...
  useEffect(() => {
    if (params.get("error") === "handoff_failed") {
      setError("Consultant session setup failed. Please sign in again.");
    }
  }, [params]);
  ...
  const submit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (res.ok) {
      // If destination is cross-domain (consultant/admin) → show handoff message
      ...
      <div className="min-h-screen bg-slate-950 flex items-center justify-center p-6">
        <div className="max-w-md text-center">
          ...
          <p className="text-slate-400 text-sm">
            Transferring your session to the admin portal.
          </p>
      ...
      <main className="flex-1 flex items-center justify-center p-6 relative">
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-md">
          <div className="mb-8">
            ...
            <p className="text-sm text-slate-500 mt-1">Sign in with email</p>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <Link href="/register"
              className="group p-4 rounded-2xl bg-slate-900/60 border border-white/5 hover:border-purple-500/40 transition-all">
