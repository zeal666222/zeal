"use client";
// ═══════════════════════════════════════════════════════════════════════════════
// Consultant Bookings — realtime-aware, accept / decline inline
// ═══════════════════════════════════════════════════════════════════════════════
import { useCallback, useEffect, useMemo, useState } from "react";
import Link from "next/link";
import {
  Calendar, Clock, User, IndianRupee, Loader2, Search,
  Check, X, MessageCircle, AlertCircle,
} from "lucide-react";
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-slate-400 mt-1">
          Accept, decline, and track every session
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          ...
          { l: "Confirmed", v: stats.confirmed, c: "text-emerald-400" },
          { l: "Completed", v: stats.completed, c: "text-slate-400" },
        ].map((s) => (
      ...
      {filtered.length === 0 ? (
        ...
        <div className="space-y-3">
          {filtered.map((b) => {
            ...
            return (
              <div key={b.id}
                className="p-4 lg:p-5 bg-slate-900/60 backdrop-blur-xl border border-white/5 rounded-2xl hover:border-purple-500/40 transition-all">
                <div className="flex items-start justify-between gap-4">
                  ...
                  <div className="text-right flex-shrink-0">
                    ...
                    <span className={"inline-block mt-1 text-[10px] uppercase font-bold tracking-wider px-2 py-0.5 rounded-full border " + (STATUS_STYLE[b.status] || STATUS_STYLE.COMPLETED)}>
                      {b.status}
                    </span>
                  </div>
                </div>
                ...
                <div className="mt-4 pt-4 border-t border-white/5 flex flex-wrap gap-2">
                  {isPending ? (
                    <>
                      <button onClick={() => action(b.id, "accept")} disabled={busy}
                      ...
                      <button onClick={() => action(b.id, "decline")} disabled={busy}
                        className="flex-1 min-w-[120px] py-2.5 rounded-xl bg-rose-500/10 hover:bg-rose-500/20 border border-rose-500/20 text-rose-400 text-xs font-black flex items-center justify-center gap-1.5 disabled:opacity-50">
                    ...
                    <>
                      <Link href={`/consultant/chat`}
                        className="flex-1 min-w-[120px] py-2.5 rounded-xl bg-white/5 hover:bg-white/10 text-slate-300 text-xs font-black flex items-center justify-center gap-1.5">
