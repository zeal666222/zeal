...
// ─────────────────────────────────────────────────────────────────────────────
// Features:
//   • Dark/light toggle via next-themes (persists, no hydration flash)
//   • Notification bell → /notifications with live unread badge (realtime)
//   • Wallet balance with realtime updates (via useWallet + useChannel)
//   • Realtime connection indicator (dev only)
// ═══════════════════════════════════════════════════════════════════════════════
...
export function TopNavBar({
  ...
  // ─── Theme (persisted by next-themes) ─────────────────────────────────────
  ...
  // ─── Wallet — realtime via useWallet hook, fallback to prop ───────────────
  ...
  // ─── Unread notifications — store + live counter ──────────────────────────
  const storeUnread = useAppStore((s) => s.unreadCount);
  const addNotification = useAppStore((s) => s.addNotification);
  const [localUnread, setLocalUnread] = useState(0);

  useEffect(() => {
    setLocalUnread(storeUnread);
  }, [storeUnread]);

  // Realtime — increment badge when a notification arrives
  ...
  // Initial unread count from server (fallback when realtime is silent)
  useEffect(() => {
    if (!userId) return;
    let cancelled = false;
  ...
  const renderThemeIcon = () => {
    if (!mounted) return <Moon size={18} className="drop-shadow-lg" />;
    return isDark
      ? <Moon size={18} className="drop-shadow-lg" />
      : <Sun size={18} className="drop-shadow-lg text-amber-400" />;
  };

  return (
    <header className="fixed top-0 left-0 right-0 z-50 h-16 sm:h-20 bg-slate-950/80 dark:bg-slate-950/80 backdrop-blur-3xl border-b border-white/5 flex items-center justify-between px-3 sm:px-8">
      {/* LEFT — Theme toggle */}
