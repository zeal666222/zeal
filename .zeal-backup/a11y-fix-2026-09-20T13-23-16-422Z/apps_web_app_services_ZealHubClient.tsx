      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-[#9D7DC5]/20 via-[#533AFD]/10 to-[#9D7DC5]/20 p-6 md:p-8">
        ...
        <div className="flex flex-wrap gap-2 mt-4 relative z-10">
          {featuredCategories.map((cat, idx) => (
            <span key={idx} className={`text-xs px-3 py-1.5 rounded-full bg-gradient-to-r ${cat.color} text-white font-medium shadow-sm hover:scale-105 transition-transform cursor-pointer`} onClick={() => router.push(`/explore?category=${cat.label.toLowerCase()}`)}>
