...
export function ApplicationReviewBoard({ appId, userId }: { appId: string; userId: string }) {
  ...
  const handleReview = async (action: string | boolean) => {
    setLoading(true);
    
    // Explicitly cast string actions ("approve" / "true") into a strict boolean
    const isApproved = typeof action === "boolean" ? action : action === "approve" || action === "true";

    const res = await processApplicationAction(appId, userId, isApproved);
    if (res.success) {
      router.refresh();
    } else {
