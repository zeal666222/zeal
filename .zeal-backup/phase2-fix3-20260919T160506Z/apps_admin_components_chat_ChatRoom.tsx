"use client";
// ═══════════════════════════════════════════════════════════════════════════════
// Admin ChatRoom — human + AI conversations, streaming, human-friendly errors
// ═══════════════════════════════════════════════════════════════════════════════
import { useEffect, useRef, useState } from "react";
import { useRouter } from "next/navigation";
import {
  ArrowLeft, Send, Loader2, ShieldCheck, Sparkles, CheckCheck, AlertCircle,
} from "lucide-react";
...
// Never expose provider text, status codes, or stack traces to users.
function friendlyChatError(raw: string | null | undefined): string {
  ...
  if (s.includes("not a participant") || s.includes("forbidden") || s.includes("403")) {
...
export function ChatRoom({
  conversationId, currentUserId, partnerId, partnerName,
  partnerAvatar, partnerIsOnline, isAI = false,
}: Props) {
  const router = useRouter();
  const { messages, isLoading, isSending, error, send, sendToAI } = useChat({
  ...
  useEffect(() => {
    void fetch(`/api/consultant/chat/${conversationId}/read`, { method: "POST" })
      .catch(() => {});
  }, [conversationId, messages.length]);

  // Surface any hook-level error as a friendly message
  useEffect(() => {
          ...
          <div className="flex items-center gap-1.5 text-[10px] font-bold">
            {showTyping ? (
              ...
              <>
                ...
                <span className="text-emerald-400">{isAI ? "AI · 24/7" : "Online"}</span>
              </>
            ) : (
