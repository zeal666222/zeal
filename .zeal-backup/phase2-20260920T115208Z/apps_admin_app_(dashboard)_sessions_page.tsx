...
import {
  Radio, Loader2, IndianRupee, Clock, User, Sparkles,
} from "lucide-react";
...
export default function LiveSessionsPage() {
  ...
  const totalAccrued = items.reduce(
    (s, it) => s + (it.durationSeconds / 60) * it.rate,
    0,
  );
        ...
        <div className="space-y-3">
          {items.map((s) => {
            ...
            return (
              <div
                key={s.id}
                className="p-4 bg-slate-900/60 border border-emerald-500/20 rounded-2xl"
              >
...
function KPI({ label, value, accent }: { label: string; value: string | number; accent: string }) {
  return (
    <div className="p-4 bg-slate-900/60 border border-white/5 rounded-2xl">
      ...
      <p className={`text-2xl font-black font-mono mt-1 ${accent}`}>
