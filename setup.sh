#!/usr/bin/env bash
# ==============================================================================
# PROJECT ZEAL — RLS BYPASS & CONSULTANT AUTH UPGRADE
# ==============================================================================
set -euo pipefail
...
echo -e "${INFO} 1. Deploying Admin-Bypass Auth Controller (apps/web/actions/auth.ts)..."
...
echo -e "${INFO} 2. Verifying Types and Compiling..."
if npx tsc --noEmit --project apps/web/tsconfig.json; then
    npm run build
    git add -A
    git commit -m "fix(zeal): implement admin client RLS bypass to guarantee consultant profile initialization during registration" || true
    git push -u origin main
    echo -e "${SUCCESS} ====================================================================="
    echo -e "${SUCCESS} REGISTRATION BUG OBLITERATED & HIGH-END AUTH DEPLOYED!"
    echo -e "${SUCCESS} ====================================================================="
else
